...
画面側にもそのまま流れる。ブラウザは `/api/events`（SSE）で更新を受け取り、

- **開いている会話が変わった** … 読書位置を保ったまま読み直し、右下に通知を出す
  （削除された場合は本文を消さず、削除された旨だけ知らせる）
- **一覧を先頭・上端で見ている** … そのまま差し替える（操作は不要）
- **スクロール中・検索中** … 勝手に動かさず「N 件の更新」を一覧の見出しに出す。押すと読み直す

サイドバーの件数と期間も追従し、監視の状態（緑=監視中 / 灰=オフ / 赤=停止中）を表示する。
